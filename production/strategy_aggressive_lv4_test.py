"""AGGRESSIVE LV4 Extended Test - 31 months (same months as aggressive/LV2 for comparison)."""
...
print(f"AGGRESSIVE LV4 EXTENDED TEST - {len(test_months)} months")
...
print(f"LV4: RR=6.5, SL=0.8xATR, BE=1.1R, Trail=2.5R, scan_every=6 bars, LIQ_SAFETY_ROE={strat.LIQ_SAFETY_ROE}%")
...
# === Final Summary ===
print(f"\n\n{'='*110}")
print(f"  AGGRESSIVE LV4 SUMMARY ({len(results)} months)")
print(f"{'='*110}")
...
try:
    ...
    # We leave empty and only compare LV4 vs Aggressive here; LV2 comparison done separately
except Exception:
    pass

print(f"\nAGGRESSIVE (V15r2) vs AGGRESSIVE LV4 comparison:")
...
print(f"LV4:        {wins}/{len(results)} profitable, avg {avg_ret:+.2f}%")

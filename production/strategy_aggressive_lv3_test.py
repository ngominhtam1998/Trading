"""AGGRESSIVE LV3 Extended Test - 31 months (same months as aggressive/LV2 for comparison)."""
...
import strategy_aggressive_lv3 as lv3
...
print(f"AGGRESSIVE LV3 EXTENDED TEST - {len(test_months)} months")
print(f"Params: pos={lv3.POSITION_PCT}% of equity, daily_limit={lv3.DAILY_LOSS_LIMIT}%, "
      f"min_score={lv3.MIN_SCORE}, max_lev={lv3.MAX_LEVERAGE}x, max_conc={lv3.MAX_CONCURRENT}")
print(f"Min notional: ${lv3.MIN_NOTIONAL} | Max vol pct: {lv3.MAX_VOL_PCT}%")
print(f"LV3: RR=5.5, SL=0.9xATR, BE=0.9R, Trail=2.0R, scan_every=8 bars, LIQ_SAFETY_ROE={lv3.LIQ_SAFETY_ROE}%")
print(f"Lev thresholds: 25x->{lv3.get_liquidation_threshold(25):.1f}%, 20x->{lv3.get_liquidation_threshold(20):.1f}%, "
      f"15x->{lv3.get_liquidation_threshold(15):.1f}%, 10x->{lv3.get_liquidation_threshold(10):.1f}%, "
      f"5x->{lv3.get_liquidation_threshold(5):.1f}%")

all_symbols = lv3.get_all_symbols()
...
for year, month in test_months:
    ...
    print(f"\n>>> {label}...")
    btc_daily = lv3.fetch_btc_daily(start_dt, end_dt)
    ...
    for coin in candidates:
        try:
            df15 = lv3.fetch_klines_range(coin, "15m", start_dt, end_dt)
            df1h = lv3.fetch_klines_range(coin, "1h", start_dt, end_dt)
    ...
    trades, final_cap, max_conc, total_vol, liq_count, peak_eq, trough_eq = lv3.backtest_portfolio(coin_data, btc_daily)
    r = lv3.report(trades, final_cap, max_conc, total_vol, liq_count, label, show_coins=False)
    if r:
...
# === Final Summary ===
print(f"\n\n{'='*110}")
print(f"  AGGRESSIVE LV3 SUMMARY ({len(results)} months)")
print(f"{'='*110}")
...
try:
    ...
    # We leave empty and only compare LV3 vs Aggressive here; LV2 comparison done separately
except Exception:
    pass

print(f"\nAGGRESSIVE (V15r2) vs AGGRESSIVE LV3 comparison:")
...
print(f"LV3:        {wins}/{len(results)} profitable, avg {avg_ret:+.2f}%")

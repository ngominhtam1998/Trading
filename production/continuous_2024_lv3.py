"""LV3 — Single continuous compounding backtest Jan 2024 -> Jun 2026.
...
import strategy_aggressive_lv3 as strat
...
print(f"LV3 AGGRESSIVE — CONTINUOUS COMPOUNDING {START_DT.strftime('%b %Y')} -> {END_DT.strftime('%b %Y')}")
print(f"Starting capital: ${strat.TOTAL_CAPITAL} | Position: {strat.POSITION_PCT}% of equity | Max lev: {strat.MAX_LEVERAGE}x")
print(f"RR=5.5, SL=0.9xATR, BE=0.9R, Trail=2.0R | Min notional: ${strat.MIN_NOTIONAL}\n")
...
print(f"\n{'='*80}")
print(f"  LV3 AGGRESSIVE — FINAL RESULT")
print(f"{'='*80}")

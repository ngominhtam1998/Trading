"""V6-3M: V6 strategy with 3m position monitoring.

Key insight: 15m backtest has look-ahead bias — within a 15m bar, it checks
SL (old price) before moving BE, so it misses "wick hits BE then reverses".
In live, exchange SL triggers on ANY tick. 3m monitoring is MORE realistic.

Problem with naive 3m: BE/trail trigger too early on 3m wicks, then price
reverses to hit BE → many small losses.

Solution: BE/trail moves on CLOSE only (not high/low). This filters wicks:
  - SL/TP exits: check high/low (realistic — exchange triggers on any tick)
  - BE/trail moves: check CLOSE (only move BE when bar CLOSES above threshold)

Entry signals: still 15m (same as v6). Position management: 3m bars.
"""
...
TOTAL_CAPITAL = 1000.0; MAX_CONCURRENT = 25
POSITION_PCT = 22.0
DAILY_LOSS_LIMIT = 18.0
MIN_SCORE = 2
MAX_LEVERAGE = 22
COINS_PER_MONTH = 50
MIN_NOTIONAL = 5.0
...
# === 1M MONITORING PARAMS (tuned from v6_3m: MC10, BE0.5, DLL8, TR2.0) ===
BARS1_PER_15 = 15              # 15 x 1m = 15m
BE_R = 0.5                    # move BE when bar CLOSES at 0.5R profit
...
def add_indicators(df):
    ...
    df["body_pct"] = abs(df["body"]) / df["close"] * 100  # body as % of price
...
def decide_v15(row, wd, htf_trend, btc_regime):
    ...
    b = row["body"]; b_pct = row.get("body_pct", abs(b)/row["close"]*100)
    vs = v > av * 1.5
    slope50 = row["ema50_slope"]
    adx = row["adx14"]
    up = e9 > e21; dn = e9 < e21
    sup = e9 > e21 > e50; sdn = e9 < e21 < e50
    above200 = e50 > e200; below200 = e50 < e200
    ...
    if adx > 25: score += 1
    if direction == "LONG" and above200 and e200 > 0: score += 1
    if direction == "SHORT" and below200 and e200 > 0: score += 1
...
def backtest_portfolio(coin_data, btc_daily, max_hold=48, start_bar=None, monitor_every=1):
    """V6-1M: 1m position monitoring + 15m entry signals.
    SL/TP/BE/trail all on 1m CLOSE (filters wicks, realistic).
    """
    # === Build 15m indicator dfs (for entries) ===
    dfs15 = {}
    for coin, data in coin_data.items():
        dfs15[coin] = add_indicators(data["15m"].copy())
    # === Build 3m price dfs (for position monitoring) ===
    dfs1 = {}
    for coin, data in coin_data.items():
        dfs1[coin] = data["1m"].copy()
    # === Build HTF maps (1h) ===
    htf_maps = {}
    for coin, data in coin_data.items():
    ...
    # === Build 3m->15m bar index mapping per coin ===
    bar1_to_15 = {}
    for coin in dfs1:
        df3 = dfs1[coin]; df15 = dfs15[coin]
        time15 = {t: i for i, t in enumerate(df15["open_time"])}
        mapping = []
        for t3 in df3["open_time"]:
            t15 = t3.floor("15min")
            mapping.append(time15.get(t15, None))
        bar1_to_15[coin] = mapping

    min_bars1 = min(len(dfs1[c]) for c in dfs1)
    WINDOW_SIZE_15 = 50; DECISION_EVERY_15 = 4
    if start_bar is None:
        start_bar = WINDOW_SIZE_15 * BARS1_PER_15
    # If start_bar is in 15m units (from cache), convert to 3m
    if start_bar < min_bars1 // BARS1_PER_15:
        start_bar = start_bar * BARS1_PER_15
    ...
    for bi in range(start_bar, min_bars1):
        # === Daily halt check (1m bars) ===
        if bi - day_start_bar >= DAILY_HALT_BARS_1M:
        ...
        if not daily_halt and capital < day_start_cap * (1 - DAILY_LOSS_LIMIT/100):
            daily_halt = True; daily_halt_until = bi + DAILY_HALT_BARS_1M
            for coin in list(positions.keys()):
                try:
                    pos = positions[coin]; c1 = dfs1[coin].iloc[bi]; ep = c1["close"]
                    pp = (ep - pos["entry"]) if pos["dir"] == "LONG" else (pos["entry"] - ep)
                    ef = (pos["units"] * ep) * (FEE_PCT/100)
                    bars_held = bi - pos["entry_idx"]
                    funding = pos["units"] * pos["entry"] * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_1M)
        ...
        # === POSITION MANAGEMENT — every 1m bar ===
        # KEY: SL/TP/BE/trail all on CLOSE (filters wicks, realistic)
        for coin in list(positions.keys()):
            pos = positions[coin]; c1 = dfs1[coin].iloc[bi]
            ...
            if pos["dir"] == "LONG":
                close_pct = (c1["close"] - entry) / entry * 100
                if close_pct <= -liq_threshold_pct:
                    ep = c1["close"]; er = "LIQ"; is_liq = True
                elif c1["close"] <= sl_price:
                    ep = sl_price; er = "Trail" if pos["trail_moved"] else ("BE" if pos["be_moved"] else "SL")
                elif c1["close"] >= tp_price:
                    ep = tp_price; er = "TP"
                # BE/trail on CLOSE
                if ep is None:
                    close_profit_pct = close_pct
                    if close_profit_pct >= TRAIL_R * orig_sl and not pos["trail_moved"]:
                        pos["sl_price"] = entry * (1 + orig_sl / 100); pos["trail_moved"] = True
                        pos["be_moved"] = True
                    elif close_profit_pct >= BE_R * orig_sl and not pos["be_moved"]:
                        pos["sl_price"] = entry * (1 + 0.01); pos["be_moved"] = True
            else:
                close_pct = (entry - c1["close"]) / entry * 100
                if close_pct <= -liq_threshold_pct:
                    ep = c1["close"]; er = "LIQ"; is_liq = True
                elif c1["close"] >= sl_price:
                    ep = sl_price; er = "Trail" if pos["trail_moved"] else ("BE" if pos["be_moved"] else "SL")
                ...
                if ep is None:
                    ...
                    if close_profit_pct >= TRAIL_R * orig_sl and not pos["trail_moved"]:
                        ...
                        pos["sl_price"] = entry * (1 - 0.01); pos["be_moved"] = True
            if ep is None and bi - pos["entry_idx"] >= MAX_HOLD_1M:
                ep = c1["close"]; er = "MaxH"
            if ep:
                if is_liq:
                    np_ = -pos["margin"]
                    ...
                    if consec_sls[coin] >= 2: cooldowns[coin] = COOLDOWN_1M; consec_sls[coin] = 0
                    del positions[coin]
                else:
                    bar_qvol = c1.get("quote_volume", 0)
                    ...
                    if bar_qvol > 0 and exit_notional > bar_qvol * MAX_VOL_PCT / 100 and er not in ("MaxH",):
                        ...
                        bars_held = bi - pos["entry_idx"]
                        funding_fill = fill_units * entry * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_1M)
                        ...
                        if remaining_notional < MIN_NOTIONAL:
                            ...
                            if consec_sls.get(coin, 0) >= 2: cooldowns[coin] = COOLDOWN_1M; consec_sls[coin] = 0
                            del positions[coin]
                    else:
                        pp = (ep - entry) if pos["dir"] == "LONG" else (entry - ep)
                        ef = (pos["units"] * ep) * (FEE_PCT/100)
                        bars_held = bi - pos["entry_idx"]
                        funding = pos["units"] * entry * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_1M)
                        np_ = pos["units"] * pp - pos["entry_fee"] - ef - funding
                        exit_vol = pos["units"] * ep; total_volume += exit_vol
                        cash += pos["margin"] + np_
        ...
        # === ENTRY SCAN — only when a new 15m bar has closed ===
        sample_coin = list(dfs1.keys())[0]
        cur_15idx = bar1_to_15[sample_coin][bi]
        if cur_15idx is None: continue
        next_15idx = bar1_to_15[sample_coin][bi+1] if bi+1 < min_bars1 else None
        is_15m_close = (next_15idx is not None and next_15idx != cur_15idx)
        if not is_15m_close: continue
        if cur_15idx % DECISION_EVERY_15 != 0: continue
        if daily_halt: continue

        current_time = dfs15[list(dfs15.keys())[0]].iloc[cur_15idx]["open_time"]
        ...
        for coin in dfs15:
            ...
            if cur_15idx >= len(dfs15[coin]): continue
            row = dfs15[coin].iloc[cur_15idx]
            wd = dfs15[coin].iloc[max(0, cur_15idx - WINDOW_SIZE_15):cur_15idx + 1]
            if len(wd) < 20: continue
            t = row["open_time"]
            htf_key = t.floor("1h") - timedelta(hours=1)
            htf = htf_maps[coin].get(htf_key, None)
            opp = decide_v15(row, wd, htf, btc_regime)
        ...
        for opp in opportunities[:available_slots]:
            coin = opp["coin"]
            if coin in positions: continue
            margin = capital * POSITION_PCT / 100
            if cash < margin: continue
            try:
                c1 = dfs1[coin].iloc[bi + 1] if bi + 1 < len(dfs1[coin]) else dfs1[coin].iloc[bi]
                ep = c1["open"]
                ...
                positions[coin] = {"dir": opp["dir"], "entry": ep, "sl_price": sl, "tp_price": tp,
                    ...
                    "entry_idx": bi + 1, "margin": margin, "be_moved": False, "trail_moved": False,
    ...
    for coin in list(positions.keys()):
        try:
            pos = positions[coin]; last = dfs1[coin].iloc[-1]; ep = last["close"]
            pp = (ep - pos["entry"]) if pos["dir"] == "LONG" else (pos["entry"] - ep)
            ef = (pos["units"] * ep) * (FEE_PCT/100)
            bars_held = len(dfs1[coin]) - 1 - pos["entry_idx"]
            funding = pos["units"] * pos["entry"] * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_1M)
            np_ = pos["units"] * pp - pos["entry_fee"] - ef - funding
            exit_vol = pos["units"] * ep; total_volume += exit_vol
            cash += pos["margin"] + np_
...
if __name__ == '__main__':
    ...
    print(f"Lev thresholds: 25x->{get_liquidation_threshold(25):.1f}%, 20x->{get_liquidation_threshold(20):.1f}%, "
          ...
          f"5x->{get_liquidation_threshold(5):.1f}%")

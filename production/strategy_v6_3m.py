...
MAX_HOLD_3M = MAX_HOLD_BARS * BARS3_PER_15                  # 360
COOLDOWN_3M = 6 * BARS3_PER_15                              # 30
...
def backtest_portfolio(coin_data, btc_daily, max_hold=48, start_bar=None, monitor_every=1):
    """V6-3M: 3m position monitoring + 15m entry signals.
    BE/trail moves on 3m CLOSE only (filters wicks). SL/TP on high/low (realistic).
    """
    ...
    for bi in range(start_bar, min_bars3):
        # === Daily halt check (3m bars) ===
        if bi - day_start_bar >= DAILY_HALT_BARS_3M:
        ...
        if not daily_halt and capital < day_start_cap * (1 - DAILY_LOSS_LIMIT/100):
            daily_halt = True; daily_halt_until = bi + DAILY_HALT_BARS_3M
            for coin in list(positions.keys()):
                try:
                    pos = positions[coin]; c3 = dfs3[coin].iloc[bi]; ep = c3["close"]
                    pp = (ep - pos["entry"]) if pos["dir"] == "LONG" else (pos["entry"] - ep)
                    ef = (pos["units"] * ep) * (FEE_PCT/100)
                    bars_held = bi - pos["entry_idx"]
                    funding = pos["units"] * pos["entry"] * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_3M)
        ...
        # === POSITION MANAGEMENT — every 3m bar ===
        # KEY: SL/TP check on high/low (realistic), BE/trail on CLOSE (filters wicks)
        for coin in list(positions.keys()):
            pos = positions[coin]; c3 = dfs3[coin].iloc[bi]
            ...
            if pos["dir"] == "LONG":
                close_pct = (c3["close"] - entry) / entry * 100
                if close_pct <= -liq_threshold_pct:
                    ep = c3["close"]; er = "LIQ"; is_liq = True
                elif c3["close"] <= sl_price:
                    ep = sl_price; er = "Trail" if pos["trail_moved"] else ("BE" if pos["be_moved"] else "SL")
                elif c3["close"] >= tp_price:
                    ep = tp_price; er = "TP"
                # BE/trail on CLOSE
                if ep is None:
                ...
                close_pct = (entry - c3["close"]) / entry * 100
                if close_pct <= -liq_threshold_pct:
                    ep = c3["close"]; er = "LIQ"; is_liq = True
                elif c3["close"] >= sl_price:
                    ep = sl_price; er = "Trail" if pos["trail_moved"] else ("BE" if pos["be_moved"] else "SL")
                ...
                if ep is None:
                    ...
                    if close_profit_pct >= TRAIL_R * orig_sl and not pos["trail_moved"]:
                        ...
                        pos["sl_price"] = entry * (1 - 0.01); pos["be_moved"] = True
            if ep is None and bi - pos["entry_idx"] >= MAX_HOLD_3M:
                ep = c3["close"]; er = "MaxH"
            if ep:
                if is_liq:
                    np_ = -pos["margin"]
                    ...
                    if consec_sls[coin] >= 2: cooldowns[coin] = COOLDOWN_3M; consec_sls[coin] = 0
                    del positions[coin]
                else:
                    bar_qvol = c3.get("quote_volume", 0)
                    ...
                    if bar_qvol > 0 and exit_notional > bar_qvol * MAX_VOL_PCT / 100 and er not in ("MaxH",):
                        ...
                        bars_held = bi - pos["entry_idx"]
                        funding_fill = fill_units * entry * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_3M)
                        ...
                        if remaining_notional < MIN_NOTIONAL:
                            ...
                            if consec_sls.get(coin, 0) >= 2: cooldowns[coin] = COOLDOWN_3M; consec_sls[coin] = 0
                            del positions[coin]
                    else:
                        pp = (ep - entry) if pos["dir"] == "LONG" else (entry - ep)
                        ef = (pos["units"] * ep) * (FEE_PCT/100)
                        bars_held = bi - pos["entry_idx"]
                        funding = pos["units"] * entry * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_3M)
                        np_ = pos["units"] * pp - pos["entry_fee"] - ef - funding
                        exit_vol = pos["units"] * ep; total_volume += exit_vol
                        cash += pos["margin"] + np_
        ...
        sample_coin = list(dfs3.keys())[0]
        cur_15idx = bar3_to_15[sample_coin][bi]
        if cur_15idx is None: continue
        next_15idx = bar3_to_15[sample_coin][bi+1] if bi+1 < min_bars3 else None
        ...
        for opp in opportunities[:available_slots]:
            ...
            try:
                c3 = dfs3[coin].iloc[bi + 1] if bi + 1 < len(dfs3[coin]) else dfs3[coin].iloc[bi]
                ep = c3["open"]
    ...
    for coin in list(positions.keys()):
        try:
            pos = positions[coin]; last = dfs3[coin].iloc[-1]; ep = last["close"]
            pp = (ep - pos["entry"]) if pos["dir"] == "LONG" else (pos["entry"] - ep)
            ef = (pos["units"] * ep) * (FEE_PCT/100)
            bars_held = len(dfs3[coin]) - 1 - pos["entry_idx"]
            funding = pos["units"] * pos["entry"] * FUNDING_RATE/100 * (bars_held // FUNDING_INTERVAL_3M)
            np_ = pos["units"] * pp - pos["entry_fee"] - ef - funding
            exit_vol = pos["units"] * ep; total_volume += exit_vol
            cash += pos["margin"] + np_

"""AGGRESSIVE LV3 - Even higher risk/reward than LV2.

Based on strategy_aggressive_lv2 with changes to push for MORE reward:
1. RR increased to 5.5 (was 4.5) — much bigger winners, accept much lower WR
2. SL tighter at 0.9x ATR (was 1.1) — less risk per trade, more leverage room
3. BE later at 0.9R (was 0.7R) — let winners breathe longer before protecting
4. Trail from 2.0R (was 1.5R) — trail later, ride even bigger trends
5. POSITION_PCT = 12.0 (was 9.0) — bigger position size, more compounding
6. MAX_CONCURRENT = 15 (was 12) — more concurrent positions
7. MAX_LEVERAGE = 15 (was 12) — allow higher leverage
8. DAILY_LOSS_LIMIT = 10.0 (was 7.0) — accept bigger daily drawdowns
9. LIQ_SAFETY_ROE = 55.0 (was 50.0) — allow SL closer to liquidation
10. MIN_SCORE = 4 (was 5) — take even lower-confidence trades
11. DECISION_EVERY = 8 (was 12) — scan entries more frequently (every 2h vs 3h)

Goal: significantly higher return than LV2, accept MaxDD up to 30-50%,
still 0 liquidation. This is VERY HIGH RISK — only use if you can tolerate
large drawdowns. Expect more losing months than LV2.
"""
...
TOTAL_CAPITAL = 1000.0; MAX_CONCURRENT = 15  # was 12
POSITION_PCT = 12.0       # was 9.0 — bigger position
DAILY_LOSS_LIMIT = 10.0   # was 7.0 — accept bigger daily DD
MIN_SCORE = 4             # was 5 — take lower-confidence trades
MAX_LEVERAGE = 15         # was 12
COINS_PER_MONTH = 50
MIN_NOTIONAL = 5.0
MAX_VOL_PCT = 10.0
LIQ_SAFETY_ROE = 55.0     # was 50.0 — allow SL closer to liq
...
def decide_v15(row, wd, htf_trend, btc_regime):
    """LV3 decision — same structure as LV2 but riskier params."""
    ...
    # === LV3: even higher leverage tiers ===
    if a < 0.5: lev = 25
    elif a < 0.8: lev = 20
    elif a < 1.2: lev = 15
    else: lev = 8
    lev = min(lev, MAX_LEVERAGE)  # MAX_LEVERAGE=15
    if is_neutral: lev = min(lev, 10)  # was 7, allow more in neutral
    # === LV3: even tighter SL (0.9x ATR), much higher RR (5.5) ===
    sl_mult = 0.9; rr = 5.5; sl_pct = max(sl_mult * a, 0.3)
    direction = None
    ...
    # --- LIQUIDATION SAFETY (LIQ_SAFETY_ROE=55, more aggressive) ---
    ...
    score = min(score, 10)
    if score < MIN_SCORE: return None  # MIN_SCORE=4 (was 5)
...
def backtest_portfolio(coin_data, btc_daily, max_hold=48):
    ...
    WINDOW_SIZE = 50; DECISION_EVERY = 8  # was 12, scan more frequently (every 2h)
    htf_maps = {}
    ...
    for bi in range(WINDOW_SIZE, min_bars):
        ...
        for coin in list(positions.keys()):
            ...
            if pos["dir"] == "LONG":
                ...
                if c["low"] <= liq_price:
                    ...
                    ep = tp_price; er = "TP"
                if ep is None:
                    # LV3: BE at 0.9R (let winners breathe longer), Trail from 2.0R (ride even bigger trends)
                    if profit_pct >= 0.9 * orig_sl and not pos["be_moved"]:
                        pos["sl_price"] = entry * (1 + 0.01); pos["be_moved"] = True
                    if profit_pct >= 2.0 * orig_sl and not pos["trail_moved"]:
        ...
        # In neutral regime, cap concurrent at 10
        max_conc_now = 10 if btc_regime == "neutral" else MAX_CONCURRENT  # was 7
        opportunities = []
...
if __name__ == '__main__':
    print("strategy_aggressive_lv3 — use strategy_aggressive_lv3_test.py to run 31-month backtest")
    ...
    print(f"RR=5.5, SL=0.9xATR, BE=0.9R, Trail=2.0R, scan_every=8 bars")

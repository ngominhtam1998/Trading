"""Test v6_1m_plus single month — 1m monitoring, aggressive params.
Usage: python test_v6_1m_plus.py [YYYY] [MM] [SEED]
...
sys.argv = ["test_v6_1m_plus", "12", "777", "v6_1m", "1"]
...
# Patch cache_path to use _1m suffix (same data as v6_1m, reuse cache)
...
if __name__ == "__main__":
    ...
    print(f"V6-1M-PLUS SINGLE MONTH TEST — {y}-{m:02d} seed={seed}")
    print(f"Params: MC={strat.MAX_CONCURRENT} POS={strat.POSITION_PCT}% LEV={strat.MAX_LEVERAGE} "
          f"BE={strat.BE_R} TR={strat.TRAIL_R} DLL={strat.DAILY_LOSS_LIMIT} RR={strat.RR}")
    print("=" * 80)
    ...
    out = os.path.join(mmt.CACHE_DIR, f"results_v6_1m_plus_{y}_{m:02d}_s{seed}.json")

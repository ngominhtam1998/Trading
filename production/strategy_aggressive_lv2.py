"""AGGRESSIVE LV2 - Even higher risk/reward than aggressive (V15r2).

Based on strategy_aggressive (V15r2) with changes to push for MORE reward:
1. RR increased to 4.5 (was 3.5) — much bigger winners, accept lower WR
2. SL tighter at 1.1x ATR (was 1.3) — less risk per trade, more leverage room
3. BE later at 0.7R (was 0.5R) — let winners breathe before protecting
4. Trail from 1.5R (was 1.2R) — trail later, ride bigger trends
5. POSITION_PCT = 9.0 (was 7.0) — bigger position size, more compounding
6. MAX_CONCURRENT = 12 (was 10) — more concurrent positions
7. MAX_LEVERAGE = 12 (was 10) — allow slightly higher leverage
8. DAILY_LOSS_LIMIT = 7.0 (was 5.0) — accept bigger daily drawdowns
9. LIQ_SAFETY_ROE = 50.0 (was 45.0) — allow SL closer to liquidation
10. MIN_SCORE = 5 (was 6) — take slightly lower-confidence trades
11. DECISION_EVERY = 12 (was 16) — scan entries more frequently (every 3h vs 4h)

Goal: significantly higher return than aggressive, accept MaxDD up to 20-30%,
still 0 liquidation. This is HIGH RISK — only use if you can tolerate drawdowns.
"""
...
TOTAL_CAPITAL = 1000.0; MAX_CONCURRENT = 12  # was 10
POSITION_PCT = 9.0        # was 7.0 — bigger position
DAILY_LOSS_LIMIT = 7.0    # was 5.0 — accept bigger daily DD
MIN_SCORE = 5             # was 6 — take lower-confidence trades
MAX_LEVERAGE = 12         # was 10
COINS_PER_MONTH = 50
MIN_NOTIONAL = 5.0
MAX_VOL_PCT = 10.0
LIQ_SAFETY_ROE = 50.0     # was 45.0 — allow SL closer to liq
...
def decide_v15(row, wd, htf_trend, btc_regime):
    """LV2 decision — same structure as aggressive but riskier params."""
    ...
    # === LV2: higher leverage tiers ===
    if a < 0.5: lev = 25
    elif a < 0.8: lev = 20
    elif a < 1.2: lev = 15
    else: lev = 8
    lev = min(lev, MAX_LEVERAGE)  # MAX_LEVERAGE=12
    if is_neutral: lev = min(lev, 7)  # was 5, allow more in neutral
    # === LV2: tighter SL (1.1x ATR), much higher RR (4.5) ===
    sl_mult = 1.1; rr = 4.5; sl_pct = max(sl_mult * a, 0.3)
    direction = None
    ...
    # --- LIQUIDATION SAFETY (LIQ_SAFETY_ROE=50, more aggressive) ---
    ...
    score = min(score, 10)
    if score < MIN_SCORE: return None  # MIN_SCORE=5 (was 6)
...
def backtest_portfolio(coin_data, btc_daily, max_hold=48):
    ...
    WINDOW_SIZE = 50; DECISION_EVERY = 12  # was 16, scan more frequently
    htf_maps = {}
    ...
    for bi in range(WINDOW_SIZE, min_bars):
        ...
        for coin in list(positions.keys()):
            ...
            if pos["dir"] == "LONG":
                ...
                if c["low"] <= liq_price:
                    ...
                    ep = tp_price; er = "TP"
                if ep is None:
                    # LV2: BE at 0.7R (let winners breathe), Trail from 1.5R (ride bigger trends)
        ...
        max_conc_now = 7 if btc_regime == "neutral" else MAX_CONCURRENT  # was 5
        opportunities = []
...
if __name__ == '__main__':
    print("strategy_aggressive_lv2 — use strategy_aggressive_lv2_test.py to run 31-month backtest")
    ...
    print(f"RR=4.5, SL=1.1xATR, BE=0.7R, Trail=1.5R, scan_every=12 bars")

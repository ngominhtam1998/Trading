"""V15 COMPOUND - Higher reward, accept more risk (but no liquidation).
Based on V14 compound with targeted changes to boost returns:
1. RR increased to 3.5 (was 3.0) — bigger winners, accept lower hit rate
2. SL tighter at 1.3x ATR (was 1.5) — less risk per trade, more room for leverage
3. BE earlier at 0.5R (was 0.7R) — protect capital fast
4. Trail from 1.2R (was 1.5R) — start trailing earlier, lock more profit
5. EMA200 as SOFT filter (score bonus, not hard reject) — keeps trade count high
6. Body filter as SOFT filter (score bonus, not hard reject)
7. Keep V14 params: ADX>=20, RSI 40-65/35-60, score>=6, conc=10, pos=7%, vol 1.5x
Goal: more trades than V15r1, higher reward than V14, MaxDD ~10-15% OK, 0 LIQ.
"""
...
TOTAL_CAPITAL = 1000.0; MAX_CONCURRENT = 10  # back to V14
POSITION_PCT = 7.0        # back to V14
DAILY_LOSS_LIMIT = 5.0
MIN_SCORE = 6             # back to V14
MAX_LEVERAGE = 10
COINS_PER_MONTH = 50
MIN_NOTIONAL = 5.0
MAX_VOL_PCT = 10.0
...
def decide_v15(row, wd, htf_trend, btc_regime):
    ...
    if adx < 20: return None  # back to V14
    # BTC regime
    ...
    if is_neutral: lev = min(lev, 5)
    # === V15: tighter SL (1.3x ATR), higher RR (3.5) ===
    sl_mult = 1.3; rr = 3.5; sl_pct = max(sl_mult * a, 0.3)
    direction = None
    if up and sup and slope50 > 0.05:
        # V14 RSI bands (no tightening)
        ...
        for coin in list(positions.keys()):
            ...
            if pos["dir"] == "LONG":
                ...
                if c["low"] <= liq_price:
                    ...
                    ep = tp_price; er = "TP"
                if ep is None:
                    # V15: BE at 0.5R (was 0.7R), Trail from 1.2R (was 1.5R)
                    if profit_pct >= 0.5 * orig_sl and not pos["be_moved"]:
                        pos["sl_price"] = entry * (1 + 0.01); pos["be_moved"] = True
                    if profit_pct >= 1.2 * orig_sl and not pos["trail_moved"]:
...
if __name__ == '__main__':
    ...
    print(f"V15 COMPOUND: Optimized signal quality + risk/reward + lãi kép")
    ...
    print(f"\n\n{'='*100}")
    print(f"  V15 COMPOUND SUMMARY (11 months)")
    print(f"{'='*100}")

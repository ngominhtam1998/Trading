"""AGGRESSIVE LV6 Extended Test - 31 months (same months as aggressive/LV2 for comparison)."""
...
print(f"AGGRESSIVE LV6 EXTENDED TEST - {len(test_months)} months")
...
print(f"LV6: RR=9.0, SL=0.6xATR, BE=1.5R, Trail=3.5R, scan_every=4 bars, LIQ_SAFETY_ROE={strat.LIQ_SAFETY_ROE}%")
...
# === Final Summary ===
print(f"\n\n{'='*110}")
print(f"  AGGRESSIVE LV6 SUMMARY ({len(results)} months)")
print(f"{'='*110}")
...
try:
    ...
    # We leave empty and only compare LV6 vs Aggressive here; LV2 comparison done separately
except Exception:
    pass

print(f"\nAGGRESSIVE (V15r2) vs AGGRESSIVE LV6 comparison:")
...
print(f"LV6:        {wins}/{len(results)} profitable, avg {avg_ret:+.2f}%")

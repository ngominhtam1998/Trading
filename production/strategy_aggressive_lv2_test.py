"""AGGRESSIVE LV2 Extended Test - 31 months (same months as aggressive for comparison)."""
...
import strategy_aggressive_lv2 as lv2
...
print(f"AGGRESSIVE LV2 EXTENDED TEST - {len(test_months)} months")
print(f"Params: pos={lv2.POSITION_PCT}% of equity, daily_limit={lv2.DAILY_LOSS_LIMIT}%, "
      f"min_score={lv2.MIN_SCORE}, max_lev={lv2.MAX_LEVERAGE}x, max_conc={lv2.MAX_CONCURRENT}")
print(f"Min notional: ${lv2.MIN_NOTIONAL} | Max vol pct: {lv2.MAX_VOL_PCT}%")
print(f"LV2: RR=4.5, SL=1.1xATR, BE=0.7R, Trail=1.5R, scan_every=12 bars, LIQ_SAFETY_ROE={lv2.LIQ_SAFETY_ROE}%")
print(f"Lev thresholds: 25x->{lv2.get_liquidation_threshold(25):.1f}%, 20x->{lv2.get_liquidation_threshold(20):.1f}%, "
      f"15x->{lv2.get_liquidation_threshold(15):.1f}%, 10x->{lv2.get_liquidation_threshold(10):.1f}%, "
      f"5x->{lv2.get_liquidation_threshold(5):.1f}%")

all_symbols = lv2.get_all_symbols()
...
for year, month in test_months:
    ...
    print(f"\n>>> {label}...")
    btc_daily = lv2.fetch_btc_daily(start_dt, end_dt)
    ...
    for coin in candidates:
        try:
            df15 = lv2.fetch_klines_range(coin, "15m", start_dt, end_dt)
            df1h = lv2.fetch_klines_range(coin, "1h", start_dt, end_dt)
    ...
    trades, final_cap, max_conc, total_vol, liq_count, peak_eq, trough_eq = lv2.backtest_portfolio(coin_data, btc_daily)
    r = lv2.report(trades, final_cap, max_conc, total_vol, liq_count, label, show_coins=False)
    if r:
...
# === Final Summary ===
print(f"\n\n{'='*110}")
print(f"  AGGRESSIVE LV2 SUMMARY ({len(results)} months)")
print(f"{'='*110}")
...
# Distribution
print(f"\nReturn distribution:")
buckets = [(-50,-20),(-20,-10),(-10,0),(0,10),(10,30),(30,50),(50,100),(100,200),(200,500)]
...
print(f"\nAGGRESSIVE (V15r2) vs AGGRESSIVE LV2 comparison:")
print(f"{'Month':>12} {'Aggressive':>12} {'LV2':>10} {'Diff':>10}")
print(f"{'-'*48}")
for r in results:
...
print(f"LV2:        {wins}/{len(results)} profitable, avg {avg_ret:+.2f}%")

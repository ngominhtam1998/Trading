"""LV2 — Single continuous compounding backtest Jan 2024 -> Jun 2026.
...
import strategy_aggressive_lv2 as strat
...
print(f"LV2 AGGRESSIVE — CONTINUOUS COMPOUNDING {START_DT.strftime('%b %Y')} -> {END_DT.strftime('%b %Y')}")
print(f"Starting capital: ${strat.TOTAL_CAPITAL} | Position: {strat.POSITION_PCT}% of equity | Max lev: {strat.MAX_LEVERAGE}x")
print(f"RR=4.5, SL=1.1xATR, BE=0.7R, Trail=1.5R | Min notional: ${strat.MIN_NOTIONAL}\n")
...
print(f"\n{'='*80}")
print(f"  LV2 AGGRESSIVE — FINAL RESULT")
print(f"{'='*80}")

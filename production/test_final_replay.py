...
WARMUP_DAYS = 4               # extra history: 1h PRIMARY frame needs >=60 closed bars

# ---- RISK-BASED SIZING (anti-blowup) ----
# FINAL sizes each trade by RISK, not by a fixed margin %. The margin is computed
# so a full stop-out loses exactly risk_pct% of TOTAL_CAPITAL (see the entry loop).
# This bounds drawdown (N full-SL losses = N*risk_pct%) and is the standard
# fixed-fractional risk model. The old FIXED_CAPITAL_SIZING flag is retired.
FIXED_CAPITAL_SIZING = True   # (legacy flag, unused by the risk-based sizer)

CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_cache_final")
...
def prep_indicators(payload):
    ...
    # BTC regime per 4h bar using EMA9/21 on 4h (faster than daily, catches
    # real trends within a week instead of waiting 2+ weeks for daily EMAs).
    df4h = payload["btc"].get("4h")
    regime_by_bar = {}
    if df4h is not None and len(df4h) > 30:
        ...
        for _, r in df4h.iterrows():
            reg = "neutral"
            if not pd.isna(r["ema21"]):
                e9 = r["ema9"] if not pd.isna(r["ema9"]) else r["ema21"]
                if r["close"] > e9 > r["ema21"]:
                    reg = "bull"
                elif r["close"] < e9 < r["ema21"]:
                    reg = "bear"
            regime_by_bar[r["open_time"].normalize()] = reg
    # Fallback: daily regime (used if 4h data missing)
    dfd = payload["btc"].get("1d")
    regime_by_day = {}
    if dfd is not None and len(dfd) > 50:
        dfd = dfd.copy()
        dfd["ema9"] = dfd["close"].ewm(span=9, adjust=False).mean()
        dfd["ema21"] = dfd["close"].ewm(span=21, adjust=False).mean()
    ...
    for s, d in payload["coins"].items():
        ...
        coins[s] = c
    return btc, (regime_by_bar if regime_by_bar else regime_by_day), coins
...
def run_replay(btc, regime_by_day, coins, window_start):
    ...
    # Precompute per-symbol ms arrays + 1m timestamp->index map + numpy indicator arrays
    # The numpy arrays let us do cheap gate checks WITHOUT pandas slicing/iloc, which
    # is the #1 bottleneck (decide_mtf does .iloc[:n] + .iloc[-1] ~1M times).
    sym = {}
    for s, c in coins.items():
        ...
        # Numpy indicator arrays for fast pre-filter (avoids pandas .iloc[-1] overhead)
        for tf in ("15m", "5m", "1h"):
            df_tf = c.get(tf)
            if df_tf is not None and len(df_tf) > 25:
                d[f"{tf}_ema9"] = df_tf["ema9"].to_numpy()
                d[f"{tf}_ema21"] = df_tf["ema21"].to_numpy()
                d[f"{tf}_ema50"] = df_tf["ema50"].to_numpy()
            else:
                d[f"{tf}_ema9"] = None
                d[f"{tf}_ema21"] = None
                d[f"{tf}_ema50"] = None
        # 15m gate arrays (used as the CONFIRM frame in FINAL)
        d15_df = c["15m"]
        d["15m_adx14"] = d15_df["adx14"].to_numpy()
        d["15m_slope"] = d15_df["ema50_slope"].to_numpy()
        d["15m_atr_pct"] = d15_df["atr_pct"].to_numpy()
        # 1h gate arrays (PRIMARY frame in FINAL): trend + ADX + slope + atr
        if c.get("1h") is not None and len(c["1h"]) > 25:
            d1h_df = c["1h"]
            d["1h_adx14"] = d1h_df["adx14"].to_numpy()
            d["1h_slope"] = d1h_df["ema50_slope"].to_numpy()
            d["1h_atr_pct"] = d1h_df["atr_pct"].to_numpy()
        else:
            d["1h_adx14"] = None; d["1h_slope"] = None; d["1h_atr_pct"] = None
        # 1m ATR for trail
        d["1m_atr_pct"] = c["1m"]["atr_pct"].to_numpy()
        d["1m_atr14"] = c["1m"]["atr14"].to_numpy()
        # 5m ATR for trail (replaces expensive _atr_5m_pct_from_1m resample)
        d["5m_atr_pct"] = c["5m"]["atr_pct"].to_numpy()
        # 1m body for opposing candles check
        d["1m_open"] = c["1m"]["open"].to_numpy()
        sym[s] = d

    _replay_start_time = time.time()
    ...
    gate_stats = {}
    equity_curve = []        # list of (day_idx, equity, n_trades_so_far, n_open_pos)

    total_bars = len(btc1_ms)
    progress_interval = 5000  # print progress every 5000 1m bars (~3.5 days)
    for mi in range(total_bars):
        ...
        # Progress bar
        if mi % progress_interval == 0 and mi > 0:
            pct = mi / total_bars * 100
            elapsed = time.time() - _replay_start_time
            remaining = elapsed / mi * (total_bars - mi) if mi > 0 else 0
            print(f"  [progress] {pct:.1f}% | bar {mi}/{total_bars} | "
                  f"elapsed {elapsed:.0f}s | ETA {remaining:.0f}s | "
                  f"trades={len(trades)} open={len(positions)}", flush=True)
        ...
        if day != cur_day:
            cur_day = day
            day_start_cap = capital
            # snapshot equity at the start of each new day
            equity_curve.append({
                "day": str(day.normalize().date()),
                "cash": cash,
                "mtm": cash + sum(p["margin"] + ((p["last_close"] - p["entry"]) * p["units"]
                                if p["dir"] == "LONG" else (p["entry"] - p["last_close"]) * p["units"])
        ...
        for s in list(positions.keys()):
            ...
            if ep is not None:
                ...
                del positions[s]
                continue
            # trailing on 1m close — FAST inline version (avoids pandas slice + _atr_5m_pct_from_1m resample)
            # Tracks running peak/trough close in pos dict; reads ATR from precomputed numpy arrays.
            cur_close = cl
            atr_1m_pct = S_["1m_atr_pct"][idx]
            if atr_1m_pct is None or np.isnan(atr_1m_pct) or atr_1m_pct <= 0:
                atr_1m_pct = pos["orig_sl_pct"]
            # 5m ATR: use the 5m bar that contains this 1m bar (approximation of _atr_5m_pct_from_1m)
            i5_now = last_closed_idx(S_["t5"], now_close, INTERVAL_MS["5m"]) if len(S_["t5"]) else -1
            if i5_now >= 0 and i5_now < len(S_["5m_atr_pct"]):
                atr_5m_pct = S_["5m_atr_pct"][i5_now]
                if atr_5m_pct is None or np.isnan(atr_5m_pct) or atr_5m_pct <= 0:
                    atr_5m_pct = atr_1m_pct * 2.24
            else:
                atr_5m_pct = atr_1m_pct * 2.24
            # LET-WINNERS-RUN trail (mirrors strategy_final.compute_trail_sl):
            #   * NO early BE — original SL stands until +TRAIL_START_R.
            #   * Trail engages at +1.5R, locks LOCK_PROFIT_R floor, WIDE ATR trail.
            trail_distance_pct = max(S.TRAIL_MIN_PCT, S.TRAIL_ATR_MULT * atr_5m_pct)
            lock_pct = S.LOCK_PROFIT_R * pos["orig_sl_pct"]
            new_sl = None; be_moved = pos["be_moved"]; trail_moved = pos["trail_moved"]
            entry = pos["entry"]; orig_sl = pos["orig_sl_pct"]
            if d == "LONG":
                if pos.get("peak_close") is None or cur_close > pos["peak_close"]:
                    pos["peak_close"] = cur_close
                peak = pos["peak_close"]
                profit_pct = (cur_close - entry) / entry * 100
                if profit_pct >= S.TRAIL_START_R * orig_sl:
                ...
                if pos.get("trough_close") is None or cur_close < pos["trough_close"]:
                    pos["trough_close"] = cur_close
                trough = pos["trough_close"]
        ...
        # Regime: find the most recent 4h bar <= now (or daily fallback).
        # regime_by_day is actually regime_by_bar (4h keys) when 4h data exists.
        # Keys are normalized 4h timestamps; find the latest one <= now_close.
        regime = "neutral"
        if regime_by_day:
            # Keys are Timestamps; find the latest <= now
            now_ts = pd.Timestamp(now_close, unit="ms")
            best_key = None
            for k in regime_by_day:
                if k <= now_ts and (best_key is None or k > best_key):
                    best_key = k
            if best_key is not None:
                regime = regime_by_day[best_key]
        ...
        for s, S_ in sym.items():
            ...
            # FINAL needs 1h PRIMARY (>=60 closed 1h bars), 15m CONFIRM, 5m ENTRY.
            if i5 < 60 or i15 < 60 or i1h < 60 or S_["1h_ema9"] is None:
                continue

            # ---- FAST PRE-FILTER (FINAL: 1h primary -> 15m confirm) ----
            # Mirrors the FIRST gates of decide_mtf so it can only SKIP what
            # decide_mtf would also reject (never skips a valid setup). The full
            # pullback / reversal / regime / score gates run inside decide_mtf.
            skip = True
            e9_h = S_["1h_ema9"][i1h]; e21_h = S_["1h_ema21"][i1h]; e50_h = S_["1h_ema50"][i1h]
            if e9_h > e21_h > e50_h:
                t_pri = "up"
            elif e9_h < e21_h < e50_h:
                t_pri = "down"
            else:
                t_pri = "flat"
            if t_pri != "flat":
                adxh = S_["1h_adx14"][i1h]; slopeh = S_["1h_slope"][i1h]; atrh = S_["1h_atr_pct"][i1h]
                if (not np.isnan(adxh) and adxh >= S.ADX_MIN_15M and
                    abs(slopeh) >= S.SLOPE_MIN_15M and
                    not np.isnan(atrh) and atrh <= 2.5):
                    direction = "LONG" if t_pri == "up" else "SHORT"
                    # CONFIRM = 15m: not opposing + momentum aligned
                    e9_15 = S_["15m_ema9"][i15]; e21_15 = S_["15m_ema21"][i15]; e50_15 = S_["15m_ema50"][i15]
                    if e9_15 > e21_15 > e50_15:
                        ...
                        t_conf = "flat"
                    if not (direction == "LONG" and t_conf == "down") and \
                       not (direction == "SHORT" and t_conf == "up"):
                        if direction == "LONG" and e9_15 > e21_15:
                            skip = False
                        elif direction == "SHORT" and e9_15 < e21_15:
                            skip = False
            if skip:
                continue
            # ---- END PRE-FILTER ----

            # Frame remap: PRIMARY=1h (d15 slot), CONFIRM=15m (d5 slot),
            # ENTRY/PULLBACK=5m (d1 slot), not-oppose higher=1h (d1h slot).
            d_pri = S_["c"]["1h"].iloc[:i1h + 1]
            d_conf = S_["c"]["15m"].iloc[:i15 + 1]
            d_entry = S_["c"]["5m"].iloc[:i5 + 1]
            opp = S.decide_mtf(s, d_pri, d_conf, d_entry, d_pri, btc_ctx, 0.0, gate_stats)
            if opp:
        ...
        for s, opp, idx in opps[:slots]:
            if s in positions:
                continue
            # RISK-BASED sizing (anti-blowup): size the notional so that a full
            # stop-out loses exactly risk_pct% of TOTAL_CAPITAL. margin = notional
            # / lev. This decouples $ risk from leverage and caps drawdown:
            # N consecutive full-SL losses = N * risk_pct %, cannot blow up.
            risk_pct = opp.get("risk_pct", S.RISK_PCT)
            sl_pct = opp["sl"]
            if sl_pct <= 0:
                continue
            risk_dollars = S.TOTAL_CAPITAL * risk_pct / 100.0
            notional = risk_dollars * 100.0 / sl_pct       # full SL loses risk_dollars
            margin = notional / opp["lev"]
...
def report(trades, final_cap, liq_count, label, peak_eq=None, trough_eq=None):
    ...
    sizing_mode = "RISK-BASED (anti-blowup)"
    print(f"SIZING: {sizing_mode}  |  risk/trade = {S.RISK_PCT_LOW}-{S.RISK_PCT_HIGH}% of ${S.TOTAL_CAPITAL:.0f}")
...
if __name__ == "__main__":
    ...
    print(f"FINAL REPLAY TEST — realistic intrabar fills, risk-based sizing")
    ...
    print(f"  Sizing: RISK-BASED ({S.RISK_PCT_LOW}-{S.RISK_PCT_HIGH}% of ${S.TOTAL_CAPITAL:.0f} per trade)")
    ...
    trades, final_cap, liq_count, gate_stats, peak_eq, trough_eq, \
        peak_eq_ts, trough_eq_ts, max_dd_seen, max_dd_seen_ts, max_dd_seen_peak, max_dd_seen_trough, \
        equity_curve = run_replay(
        btc, regime_by_day, coins, window_start)
    report(trades, final_cap, liq_count, f"GLM replay {DAYS}d / {len(coins)} symbols",
           peak_eq, trough_eq)

    # Daily equity curve (verify the "from $500 -> $14000 in 13d" claim)
    print(f"\n=== DAILY EQUITY CURVE (start of each UTC day) ===")
    print(f"{'Day':<12} {'Cash':>12} {'MTM':>12} {'Open':>5} {'Trades':>7}")
    for e in equity_curve:
        print(f"{e['day']:<12} ${e['cash']:>10.2f} ${e['mtm']:>10.2f} {e['n_open']:>5} {e['n_trades']:>7}")

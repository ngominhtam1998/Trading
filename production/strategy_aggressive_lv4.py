"""AGGRESSIVE LV4 - Higher risk/reward than LV3.

Based on strategy_aggressive_lv3 with changes to push for MORE reward:
1. RR increased to 6.5 (was 5.5) — even bigger winners
2. SL tighter at 0.8x ATR (was 0.9) — tighter stop, more leverage room
3. BE later at 1.1R (was 0.9R) — let winners breathe even longer
4. Trail from 2.5R (was 2.0R) — trail later, ride even bigger trends
5. POSITION_PCT = 15.0 (was 12.0) — bigger position size
6. MAX_CONCURRENT = 18 (was 15) — more concurrent positions
7. MAX_LEVERAGE = 18 (was 15) — higher leverage
8. DAILY_LOSS_LIMIT = 12.0 (was 10.0) — accept bigger daily drawdowns
9. LIQ_SAFETY_ROE = 60.0 (was 55.0) — SL closer to liquidation
10. MIN_SCORE = 3 (was 4) — take lower-confidence trades
11. DECISION_EVERY = 6 (was 8) — scan every 1.5h
12. Neutral regime: max 12 concurrent, 12x lev (was 10/10)

Goal: higher return than LV3, accept MaxDD up to 40-60%, high liquidation risk.
EXTREME RISK — only use if you can tolerate large drawdowns and liquidations.
"""
...
MIN_SCORE = 3             # was 4 — take lower-confidence trades
MAX_LEVERAGE = 18         # was 15
COINS_PER_MONTH = 50
MIN_NOTIONAL = 5.0
MAX_VOL_PCT = 10.0
LIQ_SAFETY_ROE = 60.0     # was 55.0 — allow SL closer to liq
...
def decide_v15(row, wd, htf_trend, btc_regime):
    """LV4 decision — same structure as LV3 but riskier params."""
    ...
    # === LV4: even higher leverage tiers ===
    if a < 0.5: lev = 25
    elif a < 0.8: lev = 20
    elif a < 1.2: lev = 15
    else: lev = 8
    lev = min(lev, MAX_LEVERAGE)  # MAX_LEVERAGE=18
    if is_neutral: lev = min(lev, 12)  # was 10, allow more in neutral
    # === LV4: even tighter SL (0.8x ATR), much higher RR (6.5) ===
    sl_mult = 0.8; rr = 6.5; sl_pct = max(sl_mult * a, 0.3)
    direction = None
    ...
    # --- LIQUIDATION SAFETY (LIQ_SAFETY_ROE=60, more aggressive) ---
    ...
    score = min(score, 10)
    if score < MIN_SCORE: return None  # MIN_SCORE=3 (was 4)
...
def backtest_portfolio(coin_data, btc_daily, max_hold=48):
    ...
    WINDOW_SIZE = 50; DECISION_EVERY = 6  # was 8, scan more frequently (every 1.5h)
    htf_maps = {}
    ...
    for bi in range(WINDOW_SIZE, min_bars):
        ...
        for coin in list(positions.keys()):
            ...
            if pos["dir"] == "LONG":
                ...
                if c["low"] <= liq_price:
                    ...
                    ep = tp_price; er = "TP"
                if ep is None:
                    # LV4: BE at 1.1R (let winners breathe longer), Trail from 2.5R (ride even bigger trends)
                    if profit_pct >= 1.1 * orig_sl and not pos["be_moved"]:
                        pos["sl_price"] = entry * (1 + 0.01); pos["be_moved"] = True
                    if profit_pct >= 2.5 * orig_sl and not pos["trail_moved"]:
        ...
        # In neutral regime, cap concurrent at 12
        max_conc_now = 12 if btc_regime == "neutral" else MAX_CONCURRENT  # was 10
        opportunities = []
...
if __name__ == '__main__':
    print("strategy_aggressive_lv4 — use continuous_2024_lv4.py to run continuous backtest")
    ...
    print(f"RR=6.5, SL=0.8xATR, BE=1.1R, Trail=2.5R, scan_every=6 bars")

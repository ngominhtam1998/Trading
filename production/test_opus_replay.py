"""REALISTIC replay test for strategy_opus — the ONLY number you should trust.
...
It reuses strategy_opus.decide_mtf / compute_trail_sl / btc_context_from_frames
DIRECTLY, so the simulated decisions are byte-for-byte what the live bot does.

USAGE (for another model / operator):
    python test_opus_replay.py [DAYS=7] [N_SYMBOLS=12] [SEED=777]

Examples:
    python test_opus_replay.py            # last 7 days, 12 top-volume symbols
    python test_opus_replay.py 14 20      # last 14 days, 20 symbols
...
CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_cache_opus")
...
def top_symbols(n):
    # Match LIVE: if the strategy pins a curated universe, test exactly that
    # (intersected with currently-listed perps) so replay == live coin set.
    ...
    if pinned:
        try:
            ...
            uni = list(pinned)
        return uni  # use the full curated universe (matches live exactly)
...
def run_replay(btc, regime_by_day, coins, window_start):
    ...
    for mi in range(len(btc1_ms)):
        ...
        if mtm > peak_eq: peak_eq = mtm
        if mtm < trough_eq: trough_eq = mtm
...
def report(trades, final_cap, liq_count, label, peak_eq=None, trough_eq=None):
    ...
    print(f"  {label}")
    print(f"{'='*78}")
    print(f"PORTFOLIO: ${S.TOTAL_CAPITAL:.0f} -> ${final_cap:.2f} ({ret:+.2f}%)")
    if peak_eq is not None:
        print(f"EQUITY (mark-to-market):  start ${S.TOTAL_CAPITAL:.0f}  |  "
              f"HIGHEST ${peak_eq:,.2f}  |  LOWEST ${trough_eq:,.2f}  |  end ${final_cap:,.2f}")
...
if __name__ == "__main__":
    ...
    print(f"OPUS REPLAY TEST — realistic intrabar fills")
    ...
    trades, final_cap, liq_count, gate_stats, peak_eq, trough_eq = run_replay(
        btc, regime_by_day, coins, window_start)
    report(trades, final_cap, liq_count, f"OPUS replay {DAYS}d / {len(coins)} symbols",
           peak_eq, trough_eq)
    if gate_stats:

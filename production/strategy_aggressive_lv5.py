"""AGGRESSIVE LV5 - Higher risk/reward than LV4.

Based on strategy_aggressive_lv4 with changes to push for MORE reward:
1. RR increased to 7.5 (was 6.5) — even bigger winners
2. SL tighter at 0.7x ATR (was 0.8) — tighter stop
3. BE later at 1.3R (was 1.1R) — let winners breathe even longer
4. Trail from 3.0R (was 2.5R) — trail later, ride even bigger trends
5. POSITION_PCT = 18.0 (was 15.0) — bigger position size
6. MAX_CONCURRENT = 20 (was 18) — more concurrent positions
7. MAX_LEVERAGE = 20 (was 18) — higher leverage
8. DAILY_LOSS_LIMIT = 15.0 (was 12.0) — accept bigger daily drawdowns
9. LIQ_SAFETY_ROE = 65.0 (was 60.0) — SL closer to liquidation
10. MIN_SCORE = 2 (was 3) — take lower-confidence trades
11. DECISION_EVERY = 4 (was 6) — scan every 1h
12. Neutral regime: max 15 concurrent, 15x lev (was 12/12)

Goal: higher return than LV4, accept MaxDD up to 50-70%, very high liquidation risk.
EXTREME RISK — only use if you can tolerate very large drawdowns and liquidations.
"""
...
TOTAL_CAPITAL = 1000.0; MAX_CONCURRENT = 20  # was 18
POSITION_PCT = 18.0       # was 15.0 — bigger position
DAILY_LOSS_LIMIT = 15.0   # was 12.0 — accept bigger daily DD
MIN_SCORE = 2             # was 3 — take lower-confidence trades
MAX_LEVERAGE = 20         # was 18
COINS_PER_MONTH = 50
MIN_NOTIONAL = 5.0
MAX_VOL_PCT = 10.0
LIQ_SAFETY_ROE = 65.0     # was 60.0 — allow SL closer to liq
...
def decide_v15(row, wd, htf_trend, btc_regime):
    """LV5 decision — same structure as LV4 but riskier params."""
    ...
    # === LV5: even higher leverage tiers ===
    if a < 0.5: lev = 25
    elif a < 0.8: lev = 20
    elif a < 1.2: lev = 15
    else: lev = 8
    lev = min(lev, MAX_LEVERAGE)  # MAX_LEVERAGE=20
    if is_neutral: lev = min(lev, 15)  # was 12, allow more in neutral
    # === LV5: even tighter SL (0.7x ATR), much higher RR (7.5) ===
    sl_mult = 0.7; rr = 7.5; sl_pct = max(sl_mult * a, 0.3)
    direction = None
    ...
    # --- LIQUIDATION SAFETY (LIQ_SAFETY_ROE=65, more aggressive) ---
    ...
    score = min(score, 10)
    if score < MIN_SCORE: return None  # MIN_SCORE=2 (was 3)
...
def backtest_portfolio(coin_data, btc_daily, max_hold=48):
    ...
    WINDOW_SIZE = 50; DECISION_EVERY = 4  # was 6, scan more frequently (every 1h)
    htf_maps = {}
    ...
    for bi in range(WINDOW_SIZE, min_bars):
        ...
        for coin in list(positions.keys()):
            ...
            if pos["dir"] == "LONG":
                ...
                if c["low"] <= liq_price:
                    ...
                    ep = tp_price; er = "TP"
                if ep is None:
                    # LV5: BE at 1.3R (let winners breathe longer), Trail from 3.0R (ride even bigger trends)
                    if profit_pct >= 1.3 * orig_sl and not pos["be_moved"]:
                        pos["sl_price"] = entry * (1 + 0.01); pos["be_moved"] = True
                    if profit_pct >= 3.0 * orig_sl and not pos["trail_moved"]:
        ...
        # In neutral regime, cap concurrent at 15
        max_conc_now = 15 if btc_regime == "neutral" else MAX_CONCURRENT  # was 12
        opportunities = []
...
if __name__ == '__main__':
    print("strategy_aggressive_lv5 — use continuous_2024_lv5.py to run continuous backtest")
    ...
    print(f"RR=7.5, SL=0.7xATR, BE=1.3R, Trail=3.0R, scan_every=4 bars")

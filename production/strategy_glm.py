"""GLM_TRADING — pullback-entry variant of opus (multi-timeframe, BTC-aware).

WHY THIS EXISTS
---------------
Opus shorts INTO the breakdown (sell-the-breakdown): once 15m trend is down and
1m momentum confirms, it enters. The bounce guard only skips if price ALREADY
bounced >= 1 ATR off the swing low — so when price is still AT the low (just
made a new leg down), opus shorts immediately and gets stopped on the next
up-tick ("cầu thang đi xuống nhấp nhô — vào rãnh xuống thì bị SL khi hồi lên").

GLM flips to SELL-THE-RALLY / BUY-THE-DIP: same trend filter, but the entry
requires price to have ALREADY pulled back off the recent swing extreme by
[PULLBACK_MIN_ATR, PULLBACK_MAX_ATR] ATRs, AND the latest 1m candle to be a
reversal candle (red for short, green for long) — i.e. the pullback just ended
and the primary trend is resuming. This banks the "nhấp nhô" instead of being
stopped by it.

The second change vs opus: dynamic universe. Opus hard-codes 30 liquid majors.
GLM can scan top N coins by 24h quote volume (configurable), so it sees more
setups without dragging in illiquid junk (a min-volume floor still applies).

Everything else (risk, sizing, SL/TP geometry, BE/ATR trail, BTC context,
funding filter, confluence score) is identical to opus — only the entry trigger
and the universe are different.
...
- backtest_portfolio(...)                             -> a CLOSE-based backtest
  kept ONLY for parity tooling; the REAL evaluation is test_glm_replay.py which
...
# === PULLBACK ENTRY (THE core change vs opus) ===
# Opus enters on the breakout (1m close in trend direction + EMA9 cross). GLM
# instead REQUIRES a pullback off the recent swing extreme, then a reversal
# candle resuming the trend. This is the "sell the rally / buy the dip" version.
PULLBACK_MIN_ATR = 0.3        # price MUST have pulled back at least this many ATRs off swing extreme
PULLBACK_MAX_ATR = 1.5        # ...but not more than this (too far = missed the move)
PULLBACK_LOOKBACK_1M = 15     # bars on 1m to find the swing extreme
PULLBACK_RSI_MIN = 40         # 1m RSI floor for SHORT entry (price not too oversold after pullback)
PULLBACK_RSI_MAX = 60         # 1m RSI ceiling for LONG entry
REVERSAL_BODY_PCT = 0.02      # reversal candle must have body >= this % of price
USE_PULLBACK_GATE = True      # master switch (False = behave like opus)
...
def decide_mtf(symbol, d15, d5, d1, d1h, btc_ctx=None, funding_rate=0.0, stats=None):
    ...
    if USE_PULLBACK_GATE:
        ...
        # RSI band: after pullback, RSI should be in mid-band (not extreme)
        rsi1 = r1["rsi14"]
        if direction == "SHORT" and (pd.isna(rsi1) or rsi1 < PULLBACK_RSI_MIN):
            return _rej(stats, "pullback_rsi_low")
        if direction == "LONG" and (pd.isna(rsi1) or rsi1 > PULLBACK_RSI_MAX):
            return _rej(stats, "pullback_rsi_high")
        # Reversal candle: latest 1m candle must close in our direction with a real body
        if r1["body_pct"] < REVERSAL_BODY_PCT:
            return _rej(stats, "no_reversal_candle")
        if direction == "SHORT" and r1["body"] >= 0:
            return _rej(stats, "no_reversal_red")
        if direction == "LONG" and r1["body"] <= 0:
            return _rej(stats, "no_reversal_green")
        # Still keep the exhaustion guard: 2/3 of last 3 candles opposing the
        # trend means the pullback is too strong, not a fade-able bounce.

"""V14 COMPOUND Extended Test - 31 months.
Tests compounding position sizing + min notional + partial fills.
"""
...
print(f"V14 COMPOUND EXTENDED TEST - {len(test_months)} months")
print(f"Params: pos={v14c.POSITION_PCT}% of equity, daily_limit={v14c.DAILY_LOSS_LIMIT}%, min_score={v14c.MIN_SCORE}, max_lev={v14c.MAX_LEVERAGE}x")
print(f"Min notional: ${v14c.MIN_NOTIONAL} | Max vol pct: {v14c.MAX_VOL_PCT}%")
print(f"Lev thresholds: 25x->{v14c.get_liquidation_threshold(25):.1f}%, 10x->{v14c.get_liquidation_threshold(10):.1f}%, 5x->{v14c.get_liquidation_threshold(5):.1f}%")

all_symbols = v14c.get_all_symbols()
...
for year, month in test_months:
    ...
    print(f"\n>>> {label}...")
    btc_daily = v14c.fetch_btc_daily(start_dt, end_dt)
    ...
    for coin in candidates:
        try:
            df15 = v14c.fetch_klines_range(coin, "15m", start_dt, end_dt)
            df1h = v14c.fetch_klines_range(coin, "1h", start_dt, end_dt)
    ...
    trades, final_cap, max_conc, total_vol, liq_count = v14c.backtest_portfolio(coin_data, btc_daily)
    r = v14c.report(trades, final_cap, max_conc, total_vol, liq_count, label, show_coins=False)
    if r: results.append(r)

# === Final Summary ===
print(f"\n\n{'='*100}")
print(f"  V14 COMPOUND SUMMARY ({len(results)} months)")
print(f"{'='*100}")
...
# Compare with V14 fixed (production)
v14_fixed = {"Dec 2022": 54.68, "Jan 2023": 20.10, "Aug 2024": 12.49, "May 2025": 68.12, "Feb 2026": 86.91,
             "May 2024": 8.88, "Oct 2025": 1.87, "Mar 2026": 70.94, "Jun 2022": 32.19, "Sep 2023": 59.00, "Feb 2025": -10.31,
             "Jan 2022": 23.47, "Feb 2022": 30.41, "Mar 2022": 22.95, "Jul 2022": -8.43, "Aug 2022": 40.57,
             "Sep 2022": 7.19, "Mar 2023": 4.87, "Jun 2023": 51.78, "Jul 2023": 29.79, "Dec 2023": 7.31,
             "Jan 2024": 0.43, "Mar 2024": 13.14, "Jul 2024": 34.25, "Sep 2024": -3.60, "Nov 2024": 16.75,
             "Apr 2025": 21.41, "Jun 2025": 32.62, "Nov 2025": 7.76, "Jan 2026": -2.09, "Apr 2026": 29.76}
print(f"\nV14 FIXED vs V14 COMPOUND comparison:")
print(f"{'Month':>12} {'FIXED':>10} {'COMPOUND':>10} {'Diff':>10}")
print(f"{'-'*45}")
for r in results:
    fx = v14_fixed.get(r['label'], 0)
    diff = r['ret'] - fx
    print(f"{r['label']:>12} {fx:>+9.2f}% {r['ret']:>+9.2f}% {diff:>+9.2f}%")
